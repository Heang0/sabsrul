...
async function initializePage() {
    // Only initialize page-specific features on index page
    if (isIndexPage()) {
        checkAuthState();
        ...
        // For other pages (like video.html), just setup basic navigation
        checkAuthState();
        setupDropdowns();
        setupMobileMenu();
...
function checkAuthState() {
    ...
    console.log('🔐 Auth check:', { hasToken: !!token, user: user });
...
function updateUserInfo(user) {
    ...
    if (userAvatar) {
        ...
        userAvatar.onerror = function() {
            this.src = 'https://via.placeholder.com/32/9333ea/ffffff?text=U';
        };
    }
...
function setupUserDropdown() {
    ...
    if (userMenuButton && userDropdownMenu) {
        ...
        const dropdownItems = userDropdownMenu.querySelectorAll('a, button');
    ...
    if (categoriesContainer) {
        categoriesContainer.innerHTML = `
            ...
            ${categories.map(category => {
                ...
                return `
                <button onclick="loadVideosByCategory('${categorySlug}')" 
    ...
    if (desktopCategories) {
        desktopCategories.innerHTML = `
            ...
            ${categories.map(category => {
                ...
                const categorySlug = typeof category === 'string' ? category : (category.slug || category.name || category.category || '');
                return `
                <a href="#" onclick="event.preventDefault(); loadVideosByCategory('${categorySlug}');" 
...
async function loadVideos(category = 'all', page = 1, append = false) {
    // Only load videos on index page
    ...
    try {
        ...
        // Store pagination info for this category
        categoryHasMore[category] = data.currentPage < data.totalPages;
...
async function loadMoreVideos() {
    ...
    // Increment page for current category
    if (!categoryPages[currentCategory]) {
        categoryPages[currentCategory] = 1;
    }
    categoryPages[currentCategory]++;
    
    await loadVideos(currentCategory, categoryPages[currentCategory], true);
}
...
function checkAuthState() {
    ...
    // Safely get DOM elements inside the function
    const userSection = document.getElementById('userSection');
    const authButtons = document.getElementById('authButtons');
    const mobileUserLinks = document.getElementById('mobileUserLinks');
...
// Also check auth state when page becomes visible (in case of tab switching)
document.addEventListener('visibilitychange', function() {
    if (!document.hidden) {
        console.log('🔄 Page visible, rechecking auth state...');
        checkAuthState();
    }
});
...
function showLoading() {
    // Only show loading on index page
    if (!isIndexPage()) return;
...
function hideLoading() {
    // Only hide loading on index page
    if (!isIndexPage()) return;
...
function showEmptyState() {
    // Only show empty state on index page
    if (!isIndexPage()) return;
...
function showErrorState(message) {
    // Only show error state on index page
    if (!isIndexPage()) return;
